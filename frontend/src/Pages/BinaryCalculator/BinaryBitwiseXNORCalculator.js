...
const BinaryBitwiseXNORCalculator = () => {
  ...
  const calculateBitwiseXNOR = () => {
    // Parse binary numbers as integers and perform bitwise XNOR
    const bitwiseXNOR = ~(parseInt(binaryNumber1, 2) ^ parseInt(binaryNumber2, 2));
    // Convert the result back to binary and update the result state
    setResult(bitwiseXNOR.toString(2));
  };
  ...
  return (
    ...
    <div className="container mt-5">
      <h2>Binary Bitwise XNOR Calculator</h2>
      <div className="mb-3">
      ...
      <button className="btn btn-primary" onClick={calculateBitwiseXNOR}>Calculate Bitwise XNOR</button>
      {result && (
        <div className="mt-4">
          <h3>Result:</h3>
          <p>{result}</p>
...
export default BinaryBitwiseXNORCalculator;
