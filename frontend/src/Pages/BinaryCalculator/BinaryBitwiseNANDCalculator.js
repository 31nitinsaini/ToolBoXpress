...
const BinaryBitwiseNANDCalculator = () => {
  ...
  const calculateBitwiseNAND = () => {
    // Parse binary numbers as integers and perform bitwise NAND
    const bitwiseNAND = ~(parseInt(binaryNumber1, 2) & parseInt(binaryNumber2, 2));
    // Convert the result back to binary and update the result state
    setResult(bitwiseNAND.toString(2));
  };
  ...
  return (
   ...
   <div className="container mt-5">
      <h2>Binary Bitwise NAND Calculator</h2>
      <div className="mb-3">
      ...
      <button className="btn btn-primary" onClick={calculateBitwiseNAND}>Calculate Bitwise NAND</button>
      {result && (
        <div className="mt-4">
          <h3>Result:</h3>
          <p>{result}</p>
...
export default BinaryBitwiseNANDCalculator;
