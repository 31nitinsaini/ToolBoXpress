...
const BinaryBitwiseORCalculator = () => {
  ...
  const calculateBitwiseOR = () => {
    // Parse binary numbers as integers and perform bitwise OR
    const bitwiseOR = parseInt(binaryNumber1, 2) | parseInt(binaryNumber2, 2);
    // Convert the result back to binary and update the result state
    setResult(bitwiseOR.toString(2));
  };
  ...
  return (
   ...
   <div className="container mt-5">
      <h2>Binary Bitwise OR Calculator</h2>
      <div className="mb-3">
      ...
      <button className="btn btn-primary" onClick={calculateBitwiseOR}>Calculate Bitwise OR</button>
      {result && (
        <div className="mt-4">
          <h3>Result:</h3>
          <p>{result}</p>
...
export default BinaryBitwiseORCalculator;
